...
#Menu URLs
newsURL = "https://menus.sodexomyway.com/BiteMenu/Menu?menuId=16653&locationId=27747017&whereami=http://masondining.sodexomyway.com/dining-near-me/news"
...
async def updateMenus():
    ...
    valid = False
    while (valid == False):
        newsP = requests.get(newsURL)
        print("Ikes Response Code: ", newsP.status_code)
        if newsP.status_code == 200: valid = True
    soupI = BeautifulSoup(newsP.content, "lxml")
...
async def viewIkes(ctx):
    await viewMenu(ctx, 'news')

@bot.command(name='rmnews')
@has_permissions(manage_channels = True)
async def rmIkes(ctx):
    await rmMenu(ctx, 'news')
